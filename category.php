...
$category = explode('/', $pathWithoutSlashes)[1];
...
try {
    ...
    $sql = "SELECT blogs.*, author.name AS author_name FROM blogs LEFT JOIN author ON blogs.author = author.username WHERE category = '$category' ORDER BY blogs.id DESC LIMIT $perPage OFFSET $offset";
...
<head>
    ...
    <link rel="canonical" href="<?= $url . '/category/' . $category ?>" />
    ...
    <meta property="og:title" co ntent="Category |
        <?= $category ?>" />
    <meta property="og:description"
        content="Welcome to <?= $mainTitle ?>, your trusted companion on the road to success in RTO exams across India. We offer a comprehensive platform designed to help you prepare effectively for your upcoming driving license and learner's permit tests. Whether you're aiming to pass the learner's test or upgrade your license, our extensive collection of RTO mock tests and practice questions provides the perfect training ground. Gain the knowledge, confidence, and skills you need to excel in your RTO exams, with instant feedback, in-depth explanations, and real exam simulations. Start your journey towards safe and informed driving today with <?= $mainTitle ?>." />
    <meta property="og:image" conten t="<?= $url ?>/asset/preview.png" />
    <meta property="og:url" content="<?= $url . '/category/' . $category ?>" />
    ...
    <meta name="twitter:description"
        content="Welcome to <?= $mainTitle ?>, your trusted companion on the road to success in RTO exams across India. We offer a comprehensive platform designed to help you prepare effectively for your upcoming driving license and learner's permit tests. Whether you're aiming to pass the learner's test or upgrade your license, our extensive collection of RTO mock tests and practice questions provides the perfect training ground. Gain the knowledge, confidence, and skills you need to excel in your RTO exams, with instant feedback, in-depth explanations, and real exam simulations. Start your journey towards safe and informed driving today with <?= $mainTitle ?>." />
    <meta name="twitter:image" conte nt="<?= $url ?>/asset/preview.png" />
    <meta name="twitter:url" content="<?= $url . '/category/' . $category ?>" />
    <?php echo $header ?>
    <title>Category |
        <?= $category ?>
    </title>
                ...
                <div class="flex flex-col items-center justify-center w-full my-6">
                    <div class="mt-4 flex flex-col items-start justify-start w-full">
                        <div class="w-full border-b-2 border-blue-400">
                            <h1 class="px-4 py-2 text-2xl font-medium">Category:
                                <?= $category ?>
                            </h1>
                        </div>
                        ...
                        <div class="w-full flex items-center justify-center gap-4 my-4">
                            ...
                            if (!($currentPage < 2)) {
                                echo '
                            <a href=' . $url . '/category/' . $category . '/' . ($currentPage - 1) . ' class="px-6 py-2 bg-blue-500 text-white hover:bg-blue-600 shadow-lg">Back</a>';
                            } ?>
                            ...
                            $sqlCount = "SELECT COUNT(*) FROM blogs WHERE category = '$category'"; // Count all blogs
                            ...
                            if ($hasNextPage) {
                                echo '<a href=' . $url . '/category/' . $category . '/' . ($currentPage + 1) . '
